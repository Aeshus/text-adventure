...
public class Console extends JFrame {
  ...
  private void initializeCaretListener() {
    CaretListener caretListen =
        caretEvent -> {

          // Get current caret position
          int caretPos = caretEvent.getDot();

          // If the caret is behind the original text & the original text is less than or equal to
          // the current text, jump it to the end of the original
          if (caretPos < consoleText.length()
              && textArea.getText().length() >= consoleText.length()) {
            textArea.setCaretPosition(consoleText.length());
          }

          // If the caret is behind the original text & the current text is less than the original,
          // jump it to the end of the current.
